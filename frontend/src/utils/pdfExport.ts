...
export function generateScorecardPDF(data: ScorecardData): Blob {
  ...
  // --- HEADER ---
  const logoHeight = 15;
  const logoWidth = logoHeight * (169 / 120);
  try {
    doc.addImage('data:image/png;base64,' + VALIRAM_LOGO_B64, 'PNG', marginL, 11, logoWidth, logoHeight);
  } catch (_) {
